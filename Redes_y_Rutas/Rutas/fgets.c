	...
	for (origen = 0; origen <= nodos; origen++){	
		for (int destino = 0; destino <= nodos; destino++){
			...
			if (origen==destino){
				...
				fprintf(archivo, "%i\t%i\t%f\n",origen,destino, prob_usr);
			}		
		}
	}
}
...
int main(int argc, char const *argv[]){
	...
	for(int text = 1; text < argc; text++){ 
		...
		while(p_block<=0.001){
			...
			double BLOCK_X_HOP[10];
			double ENV_X_HOP[10];
			
			LlenarEnlaces(canales,ENLACES,CAPACIDAD);
			llenarConCeros(10,BLOCK_X_HOP);
			llenarConCeros(10,ENV_X_HOP);
			...
			if (p_block>=0.001){
				fprintf(PBU, "%s:",argv[text]);
				for (int i = 0; i < 10; i++){
					fprintf(PBU, "%f:", BLOCK_X_HOP[i]/ENV_X_HOP[i]);	
				}
				fprintf(PBU, "\n");
			}
			/*fprintf(PBU, "********************************************************\n");
			...
			fprintf(PBU, "********************************************************\n");
			fprintf(PBU, "Origen\tDestino\tProb.Bloqueo\n");
			fprintf(PBU, "********************************************************\n");*/
